...
class RandomSampleCrop(object):
    ...
    Return:
        (img, boxes, classes)
            ...
            labels (Tensor): the class labels for each bbox
    """
    def __init__(self, patch_size, var):
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        while True:
            ...
            for i in range(1000):
                ...
                # choose the w and h to be 240-360
                w = random.uniform(self.size - self.var, self.size + self.var)
                h = w
                ...
                #if overlap.max() < iou_thre:
                #    continue

                # if rect has box in it?
                if overlap.max() == 0:
                    continue
...
class SSDAugmentation(object):
    def __init__(self, size=300, mean=(104, 117, 123), patch_size=400, var=100):
        self.mean = mean
        self.size = size
        self.augment = Compose([
            ...
            PhotometricDistort(),
            RandomRotate(),
            RandomSampleCrop(patch_size, var),
            RandomMirror(),
            RandomVerticalFlip(),
            ToPercentCoords(),
            Resize(self.size),
